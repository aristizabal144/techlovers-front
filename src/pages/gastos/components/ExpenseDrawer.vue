...
interface Emit {
  ...
  (e: 'submit', data: ExpenseData, id?: number | null): void
}
...
const formRef = ref()
const formData = ref<ExpenseData>({
  fecha: '',
  valor: null,
  metodo_pago: '',
  descripcion: '',
...
const onSubmit = async () => {
  ...
  if (valid) {
    emit('submit', formData.value, props.editingExpenseId)
  }
}
...
watch(
  ...
  ([isOpen, newInitialData]) => {
    if (isOpen && props.editingExpenseId && newInitialData) {
      formData.value = { ...newInitialData as ExpenseData }
      displayValor.value = formData.value.valor ? formatCurrency(formData.value.valor) : ''
    } else if (isOpen && !props.editingExpenseId) {
      resetForm()
    }
  },
  { deep: true }
...
const titleForm = computed(() => {
  return props.editingExpenseId ? 'Editar Gasto' : 'Nuevo Gasto'
})
</script>
          ...
          <VRow>
            ...
            <VCol cols="12" class="d-flex align-center mt-4">
              <VBtn
                ...
                :disabled="props.isSubmitting"
              >
                {{ props.editingExpenseId ? 'Actualizar' : 'Guardar' }}
              </VBtn>
              <VBtn
                color="secondary"
                variant="tonal"
